...
#Program for normalization of text written in Unicode. This is mainly geared towards Indian Scripts and English
#
# @author Neha Gupta
#
...
class Normalizer(object):
    ...
    * ZERO_WIDTH_SPACE and NO_BREAK_SPACE replaced by spaces 
    ...
    def _normalize_punctuations(self, text):
        ...
        from sacremoses
        """
        text=text.replace(Normalizer.BYTE_ORDER_MARK,'')
        text=text.replace('„', r'"')
        text=text.replace('“', r'"')
        text=text.replace('”', r'"')
        text=text.replace('–', r'-')
...
class BaseNormalizer(Normalizer):
    ...
    def __init__(self,lang,
        ...
        self.nmt_processing=nmt_processing
    ...
    def normalize(self,text):
        ...
        """
        text=text.replace(Normalizer.BYTE_ORDER_MARK,'')
        text=text.replace(Normalizer.BYTE_ORDER_MARK_2,'')
        text=text.replace(Normalizer.WORD_JOINER,'')
        text=text.replace(Normalizer.SOFT_HYPHEN,'')


        text=text.replace(Normalizer.ZERO_WIDTH_SPACE,' ') # ??
        text=text.replace(Normalizer.NO_BREAK_SPACE,' ')

        if self.lang!='bn':
            text=text.replace(Normalizer.ZERO_WIDTH_NON_JOINER, '')
            text=text.replace(Normalizer.ZERO_WIDTH_JOINER,'')
        ...
        if self.nmt_processing:
            text=self._nmt_processing(text)

        return text


    def get_char_stats(self,text):    
        print(len(re.findall(Normalizer.BYTE_ORDER_MARK,text)))
        print(len(re.findall(Normalizer.BYTE_ORDER_MARK_2,text)))
        print(len(re.findall(Normalizer.WORD_JOINER,text)))
        print(len(re.findall(Normalizer.SOFT_HYPHEN,text)))

        print(len(re.findall(Normalizer.ZERO_WIDTH_SPACE,text) ))
        print(len(re.findall(Normalizer.NO_BREAK_SPACE,text)))

        print(len(re.findall(Normalizer.ZERO_WIDTH_NON_JOINER,text)))
        print(len(re.findall(Normalizer.ZERO_WIDTH_JOINER,text)))

        #for mobj in re.finditer(Normalizer.ZERO_WIDTH_NON_JOINER,text):
        #    print text[mobj.start()-10:mobj.end()+10].replace('\n', ' ').replace(Normalizer.ZERO_WIDTH_NON_JOINER,'').encode('utf-8')
...
class EnglishNormalizer(BaseNormalizer): 
    """
    Normalizer for the English. 
    Basic Normalization will be performed in the super class, 

    """
    # def __init__(self,lang,remove_nuktas=False,nasals_mode='do_nothing',do_normalize_chandras=False,do_normalize_vowel_ending=False, nmt_processing=False):
    #     super(EnglishNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
    ...
    def normalize(self,text): 
        ...
        text = re.sub('You\'ve not','you have not',text)
        text = re.sub('we\'ve not','we have not',text)
        text = re.sub('they\'ve not','they have not',text)
        text = re.sub('hasn\'t','has not',text)
        text = re.sub('haven\'t','have not',text)
        
        text = re.sub('i\'d not','i had not',text)
        text = re.sub('he\'d not','he had not',text)
        text = re.sub('she\'d not','she had not',text)
        text = re.sub('it\'d not','it had not',text)
        text = re.sub('you\'d not','i had not',text)
        text = re.sub('we\'d not','we had not',text)
        text = re.sub('they\'d not','they had not',text)
        text = re.sub('i\'d','i had',text)
        text = re.sub('he\'d','he had',text)
        text = re.sub('she\'d','she had',text)
        text = re.sub('it\'d','it had',text)
        text = re.sub('you\'d','i had',text)
        ...
        text = re.sub('i\'ll not','i will not',text)
        text = re.sub('we\'ll not','we will not',text)
        text = re.sub('he\'ll not','he will not',text)
        text = re.sub('she\'ll not','she will not',text)
        text = re.sub('it\'ll not','it will not',text)
        text = re.sub('you\'ll not','you will not',text)
        text = re.sub('we\'ll not','we will not',text)
        text = re.sub('they\'ll not','they will not',text)
        ...
        text=super(EnglishNormalizer,self).normalize(text)
...
class DevanagariNormalizer(BaseNormalizer): 
    ...
    * replace colon ':' by visarga if the colon follows a charcter in this script 
    ...
    def __init__(self,lang='hi',remove_nuktas=False,nasals_mode='do_nothing',
            do_normalize_chandras=False,do_normalize_vowel_ending=False,nmt_processing=False):
        super(DevanagariNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)

    def normalize(self,text): 

        # START: MODIFICATION 2
        # We move the super() call to the TOP.
        # This runs all base normalization FIRST (including the modified _nmt_processing).
        ...
        text=re.sub(r'([\u0900-\u097f]):','\\1\u0903',text)
        text=re.sub(r'\u0919\u094d([\u0915-\u0918])','\u0902\\1',text)
        text=re.sub(r'\u091E\u094d([\u091A-\u091D])','\u0902\\1',text)
        text=re.sub(r'\u0923\u094d([\u091F-\u0922])','\u0902\\1',text)
        text=re.sub(r'\u0928\u094d([\u0924-\u0927])','\u0902\\1',text)

        # change varya ra ra+halant+zwj to ra_nukta+halant
        text=text.replace('\u0930\u094d\u200D','\u0931\u094d')
...
class GurmukhiNormalizer(BaseNormalizer): 
    ...
    * replace colon ':' by visarga if the colon follows a charcter in this script 
    ...
    def __init__(self,lang='pa',remove_nuktas=False,nasals_mode='do_nothing',do_normalize_chandras=False,
                ...
                nmt_processing=False):
        super(GurmukhiNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
...
class GujaratiNormalizer(BaseNormalizer): 
    ...
    * replace colon ':' by visarga if the colon follows a charcter in this script 
    ...
    def __init__(self,lang='gu',remove_nuktas=False,nasals_mode='do_nothing',do_normalize_chandras=False,
                    do_normalize_vowel_ending=False,nmt_processing=False):
        super(GujaratiNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
...
class OriyaNormalizer(BaseNormalizer): 
    ...
    * replace colon ':' by visarga if the colon follows a charcter in this script 
    ...
    def __init__(self,lang='or',remove_nuktas=False,nasals_mode='do_nothing',do_normalize_chandras=False,
                ...
                do_remap_wa=False,
                nmt_processing=False):
        super(OriyaNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
...
import unicodedata
# Assuming BaseNormalizer is defined and imported

class BengaliNormalizer(BaseNormalizer): 
    """
    Normalizer covering Bengali ('bn'), Assamese ('as'), and Meitei/Manipuri ('mni').
    
    In addition to basic normalization by the super class, this class includes:
    * Bengali/Assamese-specific nukta decomposition and ZWJ/ZWNJ handling.
    * Meitei Mayek-specific Poorna Virama standardization and Vowel Canonicalization (when lang='mni').
    """
    ...
    # --- CONSTANTS FOR MEITEI MAYEK (Unicode Block: 0xAE00–0xAEOF) ---
    
    # Meitei Mayek specific Poorna Virama (Cheikhei)
    MEITEI_POORNA_VIRAMA = '\u0AEE4'     # Meitei Mayek Cheikhei
    MEITEI_DOUBLE_POORNA_VIRAMA = '\u0AEE5' # Meitei Mayek Ahum Cheikhei

    # Meitei Vowel Canonicalization Mappings
    # L.E. sign (U+AEBB) + L.A. sign (U+AEBE) -> L.O. sign (U+AEBF)
    VOWEL_CANONICALIZATION_MEITEI = [
        ('\uAEBB\uAEBE', '\uAEBF'), 
    ]
    # -------------------------------------------------------------------
    ...
    def __init__(self,lang='bn',remove_nuktas=False,nasals_mode='do_nothing',do_normalize_chandras=False,
                 ...
                 nmt_processing=False):
        super(BengaliNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
        self.do_remap_assamese_chars=do_remap_assamese_chars

    def _normalize_meitei_mayek(self, text):
        """Applies normalization rules specific to Meitei Mayek ('mni')."""
        
        # 1. Zero-Width Character Removal (ZWJ/ZWNJ are handled globally later, but good to be explicit here)
        # text = text.replace(self.ZERO_WIDTH_JOINER, '')
        # text = text.replace(self.ZERO_WIDTH_NON_JOINER, '') 
        
        # 2. Meitei-specific Poorna Virama Standardization
        # Replace Meitei Mayek specific punctuation with generic Indic ones (\u0964)
        text = text.replace(self.MEITEI_POORNA_VIRAMA, '\u0964')
        # Canonicalizing the triple cheikhei to a single generic danda
        text = text.replace(self.MEITEI_DOUBLE_POORNA_VIRAMA, '\u0964') 
        
        # 3. Meitei Vowel Canonicalization (Two-part to single character)
        for old, new in self.VOWEL_CANONICALIZATION_MEITEI:
            text = text.replace(old, new)

        # 4. Use NFKC for canonicalizing composed characters in Meitei Mayek
    ...
    def normalize(self,text):
        ...
        # ----------------------------------------------------
        # MEITEI MAYEK SPECIFIC NORMALIZATION
        # ----------------------------------------------------
        if self.lang == 'mni':
            text = self._normalize_meitei_mayek(text)

        # ----------------------------------------------------
        # BENGALI/ASSAMESE SPECIFIC NORMALIZATION (Existing Logic)
        # ----------------------------------------------------
        ...
        # Replace Ta Halant ZWJ to Khanda Ta
        text=text.replace('\u09A4\u09CD'+BengaliNormalizer.ZERO_WIDTH_JOINER,'\u09CE')
        ...
        # Remove ZWJ excep RA+ZWJ+YA (Your existing ZWJ logic)
        text=re.sub(r'([^\u09B0])\u200D([^\u09CD])','\\1\\2',text)
        text=re.sub(r'([\u09B0])\u200D([^\u09CD])','\u09B0\\2',text)
        text=re.sub(r'([^\u09B0])\u200D([\u09CD])','\\1\u09CD',text)
        
        # Remove ZWNJ excep HALANT+ZWNJ (Your existing ZWNJ logic)
        text=re.sub(r'([^\u09CD])\u200C','\\1',text)

        # ----------------------------------------------------
        # COMMON NORMALIZATION (BaseNormalizer)
        # ----------------------------------------------------
        # common normalization for Indic scripts
...
class TamilNormalizer(BaseNormalizer): 
    ...
    * replace colon ':' by visarga if the colon follows a charcter in this script 
    ...
    def __init__(self,lang='ta',remove_nuktas=False,nasals_mode='do_nothing',
            do_normalize_chandras=False,do_normalize_vowel_ending=False,nmt_processing=False):
        super(TamilNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
...
class TeluguNormalizer(BaseNormalizer): 
    ...
    * replace colon ':' by visarga if the colon follows a charcter in this script 
    ...
    def __init__(self,lang='te',remove_nuktas=False,nasals_mode='do_nothing',
                do_normalize_chandras=False,do_normalize_vowel_ending=False,nmt_processing=False):
        super(TeluguNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
...
class KannadaNormalizer(BaseNormalizer): 
    ...
    * replace colon ':' by visarga if the colon follows a charcter in this script 
    ...
    def __init__(self,lang='kn',remove_nuktas=False,nasals_mode='do_nothing',
            do_normalize_chandras=False,do_normalize_vowel_ending=False,nmt_processing=False):
        super(KannadaNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
...
class MalayalamNormalizer(BaseNormalizer): 
    ...
    * replace colon ':' by visarga if the colon follows a charcter in this script 
    ...
    def __init__(self,lang='ml',remove_nuktas=False,nasals_mode='do_nothing',do_normalize_chandras=False,
                do_normalize_vowel_ending=False,
                do_canonicalize_chillus=False, do_correct_geminated_T=False,nmt_processing=False):
        super(MalayalamNormalizer,self).__init__(lang,remove_nuktas,nasals_mode,do_normalize_chandras,do_normalize_vowel_ending,nmt_processing)
...
class UrduNormalizer(Normalizer):
...
class LanguageNormalizer(object):
    """
...
if __name__ == '__main__': 
    ...
    if len(sys.argv)<4:
        print("Usage: python normalize.py <infile> <outfile> <language> [<replace_nukta(True,False)>] [<normalize_nasals(do_nothing|to_anusvaara_strict|to_anusvaara_relaxed|to_nasal_consonants)>] [<nmt(True,False)>]") 
        sys.exit(1)
    ...
    if len(sys.argv)>=7:
        nmt_processing=bool(sys.argv[6])


    # create normalizer
    objLanguageNormalizer=LanguageNormalizer()
    normalizer=objLanguageNormalizer.get_normalizer(language,remove_nuktas=remove_nuktas,nasals_mode=normalize_nasals,nmt_processing=nmt_processing)

    # # DO normalization 
    with codecs.open(sys.argv[1],'r',encoding='utf-8') as ifile:
